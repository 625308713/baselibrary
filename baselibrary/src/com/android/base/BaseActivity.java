 ...
 *
 */
public abstract class BaseActivity extends Activity implements IActivity,Callback,PullView,OnClickListener{
	 ...
	 * @param isVerify
	 */
	protected void submit(final int requestCode,boolean isVerify){
		submit(requestCode, isVerify, false);
	}
	 ...
	 * @param showDialog 是否显示确定按钮
	 */
	protected void submit(final int requestCode,boolean isVerify,boolean showDialog){
		//验证根据验证结果,不验证直接请求
		boolean flag = isVerify?requestVerify(requestCode):true;
		if(flag){
			 gogogo(showDialog,requestCode);
		}
	}
	 ...
	 * GoGoGo
	 */
	private void gogogo(boolean showDialog,final int requestCode){
		if(showDialog){
			DialogUtil.showSweetAlertDialog(_context, 
				getS(R.string.dialog_title_xg), getS(R.string.dialog_alert_two), 
